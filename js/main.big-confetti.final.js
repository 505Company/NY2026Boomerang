document.addEventListener("DOMContentLoaded", () => {
    ...
    const confettiFX = (() => {
        ...
        let dpr = 1; // force low DPR for performance
        let W = 0, H = 0;
        ...
        function mega() {
            // Single centered burst (lightweight)
            burst({
                count: 45,
                originX: 0.50,
                originY: 0.60,
    ...
    function maybePlayConfetti() {
        ...
        const should = isFinal; // confetti only on final card
    ...
    document.querySelectorAll(".js-next").forEach((btn) => {
        btn.addEventListener("click", () => {
            // Confetti on "Получить" (first card)
            if (btn.closest(".card--welcome")) {
                confettiFX.mega();
            }
            swipeOut(1);
        });
